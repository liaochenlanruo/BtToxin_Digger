...
sub Step2  {
	...
	if(-s "$step1_in")  {
		...
		foreach my $key(sort keys %step2_in)  {
			...
			foreach (sort keys %$temp)  {
				if((exists $step2_out{$_}) && (!($$temp{$_} =~ /vip\.hmm|cyt\.hmm|cry\.hmm|App\.hmm|Gpp\.hmm|Mcf\.hmm|Mpf\.hmm|Mpp\.hmm|Mtx\.hmm|Pra\.hmm|Prb\.hmm|Spp\.hmm|Tpp\.hmm|Vpa\.hmm|Vpb.hmm\.hmm|Xpp\.hmm/i)))  {
					delete $$temp{$_};
				}else  {
					if(!(exists $seq_ids{$_}))  {
						$seq_ids{$_} = '';
...
#	my $dir = getcwd();
	...
	if($step eq 'step1')  {
		...
		$evalue = 1e-25;
		system("/usr/local/bin/blastp -query $input -out $output -db $db -evalue $evalue -num_threads 2");
		return $output;
	}
